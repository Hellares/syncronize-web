...
/**
 * Alta de una compra, y edicion de una que quedo en BORRADOR.
 *
 * Es la misma pantalla: con `compra` arranca cargada y guarda con PUT. El
 * backend hace MERGE, pero `detalles` y `gastos` se reemplazan ENTEROS — lo
 * que no vuelve, se borra.
 */
export default function CompraForm({ compra }: { compra?: CompraDetalle }) {
  ...
  // Corre una sola vez: despues manda lo que el usuario esta escribiendo.
  useEffect(() => {
    if (!compra) return;
    ...
    setDiasCredito(compra.diasCredito ? String(compra.diasCredito) : '');
    setObservaciones(compra.observaciones ?? '');
    // 🔴 La convencion de IGV sale de la compra: reabrir con el default `true`
    // una compra cargada con el IGV por fuera cambiaba TODOS los montos al
    // guardar, con los mismos precios en pantalla.
    const incluyeIgv = compra.precioIncluyeIgv ?? true;
    setPrecioIncluyeIgv(incluyeIgv);
    setLineas((compra.detalles ?? []).map((d) => lineaDesdeDetalleGuardado(d, incluyeIgv)));
    setGastos((compra.gastos ?? []).map(gastoDesdeGuardado));
    // La cabecera arranca plegada: ya esta completa y solo roba altura.
    setCabeceraAbierta(false);
  ...
  const guardar = async () => {
    ...
    const detalles: CrearCompraLinea[] = cargadas.map((l) => {
        ...
        return {
          ...
          ...(nuevoPV > 0
            ...
            : {}),
          // Solo tienen valor al EDITAR, y ahi son obligatorios: guardar
    ...
    // 🔴 `criterio` y `categoriaGastoId` viajan de vuelta aunque la web todavia
    // no los ofrezca: guardar REEMPLAZA la lista de gastos, y un flete cargado
    // desde el app volveria sin su categoria y dejaria de sumar en el reporte.
    const gastosPayload = gastosCargados().map((g, i) => ({
      concepto: g.concepto.trim(),
      monto: numVal(g.monto),
      prorratea: g.prorratea,
      ...(g.criterio ? { criterio: g.criterio } : {}),
      ...(g.categoriaGastoId ? { categoriaGastoId: g.categoriaGastoId } : {}),
      orden: i,
    }));
    const cabecera = {
    ...
    try {
      if (compra) {
        // El PUT es un MERGE, asi que los textos van SIEMPRE —tambien vacios—:
        // omitirlos conserva lo guardado y no habria forma de borrar un numero
        // de factura escrito mal. Los gastos van siempre por lo mismo.
        const actualizada = await actualizarCompra(compra.id, {
          ...cabecera,
          tipoDocumentoProveedor: (serie.trim() || numero.trim()) ? tipoDoc : '',
          serieDocumentoProveedor: serie.trim(),
          numeroDocumentoProveedor: numero.trim(),
          observaciones: observaciones.trim(),
          gastos: gastosPayload,
        });
        router.push(`/dashboard/compras/${actualizada.id}`);
        return;
      }
      const creada = await crearCompra({
        ...cabecera,
      ...
      setError(
        ...
        (editando ? 'No se pudo guardar la compra' : 'No se pudo crear la compra'),
      );
      setGuardando(false);
    }
  };
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 flex-wrap items-center gap-3">
          ...
          <h1 className="shrink-0 text-lg font-semibold text-[#004A94]">
            {editando ? `Editar ${compra.codigo}` : 'Nueva compra'}
          </h1>
          <button
      ...
      <p className="mt-3 text-right text-xs text-gray-400">
        {editando
          ? 'Sigue en BORRADOR. Guardar reemplaza las líneas y los gastos por los de esta pantalla.'
          : 'Se crea en BORRADOR. Luego confirmás (con o sin pago) desde el detalle.'}
      </p>
    </div>
  );
}
