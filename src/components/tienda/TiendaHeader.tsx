...
function redesDe(empresa: Empresa) {
  ...
  // `grande`: el círculo de Facebook mide 20 de los 24 px del viewBox y el
  // Instagram los 24 enteros; con +4 px quedan parejos a la vista.
  const lista: { nombre: string; url: string | null; icono: React.ReactNode; grande?: boolean }[] = [
    {
      nombre: 'Facebook', url: linkDeRed('facebook', redes?.facebook || empresa.facebook), grande: true,
  ...
  return lista.filter((r): r is { nombre: string; url: string; icono: React.ReactNode; grande?: boolean } => !!r.url);
}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 md:py-3 flex flex-wrap md:flex-nowrap items-center gap-x-6 gap-y-2">
            ...
            >
              <input
                ...
                className="flex-1 min-w-0 pl-4 md:pl-5 pr-3 py-1.5 md:py-2 rounded-l-lg border-2 border-r-0 border-gray-300 bg-white text-sm text-gray-900 focus:outline-none placeholder:text-gray-500"
                ...
                <div className="flex items-center gap-3 lg:gap-6">
                  {redes.map((red) => (
                    <a key={red.nombre} href={red.url} target="_blank" rel="noopener noreferrer"
                      ...
                      <svg className={red.grande ? 'w-7 h-7' : 'w-6 h-6'} fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">{red.icono}</svg>
                    </a>
                  ))}
                </div>
              )}
                ...
                <div className="flex items-center gap-4 px-3 py-2">
                  {redes.map((red) => (
                    <a key={red.nombre} href={red.url} target="_blank" rel="noopener noreferrer"
                      ...
                      <svg className={red.grande ? 'w-6 h-6' : 'w-5 h-5'} fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">{red.icono}</svg>
                    </a>
                  ))}
                </div>
              )}
