                ...
                <div key={d.id} className={`flex items-center justify-between gap-3 px-4 py-2.5 ${d.origenComboId ? 'bg-purple-50/40' : ''}`}>
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400">
                      ...
                      {Number(d.descuento ?? 0) > 0 && <span className="text-amber-600"> · desc {fmt(d.descuento)}</span>}
                      {(d as { nivelAplicadoSnapshot?: string }).nivelAplicadoSnapshot && <span className="ml-1 rounded bg-blue-100 px-1 text-blue-700">{(d as { nivelAplicadoSnapshot?: string }).nivelAplicadoSnapshot}</span>}
                    </p>
                  </div>
                  <span className="shrink-0 text-sm font-semibold text-gray-800">{fmt(d.total)}</span>
                </div>
              ))}
            </div>
